...
public class Main {
    public static void main(String[] args) {
        ...
        rulesToAdd.add(
                Term.textToTerm(
                        "checkList([H|T]) :- ref(Ref),jpl_call(Ref, debug, ['step 1'], _), fact(H,_,1), !, checkList(T);" +
                                "ref(Ref),jpl_call(Ref, debug, ['step 2'], _), fact(H,_,0),!,fail;" +
                                "ref(Ref),jpl_call(Ref, debug, ['step 3'], _), cond(H,_,AssQ,_), fact(_,AssQ,_),!, fail;" +
                                "ref(Ref),jpl_call(Ref, debug, ['step 4'], _), cond(H,_,AssQ,_), askQuestion(H,AssQ), addAssConds(AssQ), thread_wait(updated(1),[]), fact(H,_,1), !, checkList(T);" +
                                "ref(Ref),jpl_call(Ref, debug, ['step 5'], _), !,fail"));

        //ключевая обработка
        rulesToAdd.add(Term.textToTerm("work(Answ) :- rule(Res, List), ref(Ref), jpl_call(Ref, debug, [Res], _), checkList(List), Answ = Res"));
